...
pub struct Task {
    ...
    pub parent_task_id: Option<Uuid>, // Foreign key to parent Task (subtasks)
...
ORDER BY t.created_at DESC"#,
    ...
    pub async fn find_by_id(pool: &SqlitePool, id: Uuid) -> Result<Option<Self>, sqlx::Error> {
        sqlx::query_as!(
            Task,
            r#"SELECT id as "id!: Uuid", project_id as "project_id!: Uuid", title, description, status as "status!: TaskStatus", parent_task_id as "parent_task_id: Uuid", parent_workspace_id as "parent_workspace_id: Uuid", shared_task_id as "shared_task_id: Uuid", created_at as "created_at!: DateTime<Utc>", updated_at as "updated_at!: DateTime<Utc>"
               FROM tasks
               WHERE id = $1"#,
            id
        )
    ...
    pub async fn create(
        ...
        sqlx::query_as!(
            Task,
            r#"INSERT INTO tasks (id, project_id, title, description, status, parent_task_id, parent_workspace_id, shared_task_id)
               VALUES ($1, $2, $3, $4, $5, $6, $7, $8)
               RETURNING id as "id!: Uuid", project_id as "project_id!: Uuid", title, description, status as "status!: TaskStatus", parent_task_id as "parent_task_id: Uuid", parent_workspace_id as "parent_workspace_id: Uuid", shared_task_id as "shared_task_id: Uuid", created_at as "created_at!: DateTime<Utc>", updated_at as "updated_at!: DateTime<Utc>""#,
            task_id,
            data.project_id,
            data.title,
            data.description,
    ...
    pub async fn update(
        ...
        sqlx::query_as!(
            Task,
            r#"UPDATE tasks
               SET title = $3, description = $4, status = $5, parent_task_id = $6, parent_workspace_id = $7
    ...
    pub async fn child_task_ids(
        pool: &SqlitePool,
        parent_task_id: Uuid,
    ) -> Result<Vec<Uuid>, sqlx::Error> {
        sqlx::query_scalar!(
            "SELECT id as \"id!: Uuid\" FROM tasks WHERE parent_task_id = $1",
            parent_task_id
        )
        .fetch_all(pool)
        .await
    ...
    /// Collect task IDs in the subtree rooted at `root_task_id` in post-order (children before parent).
    pub async fn subtree_task_ids_postorder(
        pool: &SqlitePool,
        root_task_id: Uuid,
    ) -> Result<Vec<Uuid>, sqlx::Error> {
        let mut out = Vec::new();
        let mut stack = vec![(root_task_id, false)];
        while let Some((cur, expanded)) = stack.pop() {
            ...
            stack.push((cur, true));
            for child in Self::child_task_ids(pool, cur).await? {
    ...
    pub async fn promote_children_to_parent_task_id<'e, E>(
        executor: E,
        from_parent_task_id: Uuid,
        to_parent_task_id: Option<Uuid>,
    ) -> Result<u64, sqlx::Error>
    where
        E: Executor<'e, Database = Sqlite>,
    {
        let result = sqlx::query!(
            "UPDATE tasks SET parent_task_id = $2, updated_at = CURRENT_TIMESTAMP WHERE parent_task_id = $1",
            from_parent_task_id,
            to_parent_task_id
        )
        .execute(executor)
